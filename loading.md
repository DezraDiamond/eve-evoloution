...
2. You want to add a model component, like a [`MotionAdapter`](animatediff.md#animatediffpipeline), to [`AnimateDiffPipeline`] which was instantiated from an existing [`StableDiffusionPipeline`]. Again, both pipelines share the same pretrained model, so it'd be a waste of memory to load an entirely new pipeline again.
...
> [!TIP]
> To switch between tasks (rather than features), use the [`~DiffusionPipeline.from_pipe`] method with the [AutoPipeline](auto_pipeline.md) class, which automatically identifies the pipeline class based on the task (learn more in the [AutoPipeline](fort-obsidian/diffusers/docs/source/en/tutorials/autopipeline.md) tutorial).

Let's start with a [`StableDiffusionPipeline`] and then reuse the loaded model components to create a [`StableDiffusionSAGPipeline`] to increase generation quality. You'll use the [`StableDiffusionPipeline`] with an [IP-Adapter](fort-obsidian/diffusers/docs/source/en/using-diffusers/ip_adapter.md) to generate a bear eating pizza.
...
- Load the cached weights into the correct pipeline [class](fort-obsidian/diffusers/docs/source/en/api/pipelines/overview.md#diffusers-summary) - retrieved from the `model_index.json` file - and return an instance of it.
