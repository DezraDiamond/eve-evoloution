...
[torch.compile](fort-obsidian/diffusers/docs/source/en/optimization/torch2.0.md#torchcompile) can speed up your pipeline even more, but the LoRA weights must be fused first and then unloaded. Typically, the UNet is compiled because it is such a computationally intensive component of the pipeline.
...
Learn more about torch.compile in the [Accelerate inference of text-to-image diffusion models](fast_diffusion.md#torchcompile) guide.
