...
By default diffusers makes use of [model cpu offloading](memory.md#model-offloading) to run the whole IF pipeline with as little as 14 GB of VRAM.
...
You can also use [`torch.compile`](fort-obsidian/diffusers/docs/source/en/optimization/torch2.0.md). Note that we have not exhaustively tested `torch.compile`
